...
page_init(void)
{
	...
	// this is the example code which marks all physical pages as free;
	//size_t i;
	//for (i = 0; i < npages; i++) {
	//	pages[i].pp_ref = 0;
	//	pages[i].pp_link = page_free_list;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	uintptr_t va_p;
	perm |= PTE_P;
	for(va_p=va_start; va_p<va_end; va_p++){
		if(va_p >= ULIM){
			user_mem_check_addr = va_p;
			return -E_FAULT;
		}
		pte_t * pte = pgdir_walk(env->env_pgdir, (void *)va_p, 0);
		if(((*pte)&perm)!=perm || pte==NULL){
			user_mem_check_addr = va_p;
			return -E_FAULT;
		}
	}
