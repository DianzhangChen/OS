...
pgfault(struct UTrapframe *utf)
{
	...
	if((err & FEC_WR)==0 
		|| (vpd[PDX(addr)] & PTE_P) ==0 
		|| (vpt[PGNUM(addr)] & PTE_COW) == 0)
	{
...
duppage(envid_t envid, unsigned pn)
{
	...
	{
		cprintf("lib/fork.c: %s: duplicate page above UTOP!\n", __FUNCTION__);
		return -1;
	}

	if((vpt[PGNUM(addr)] & PTE_P)==0)
	{
		cprintf("lib/fork.c %s: page table not present!\n", __FUNCTION__);
		return -1;
	}

	if((vpd[PDX(addr)] & PTE_P)==0)
	{
		cprintf("lib/fork.c %s: page directory not present!\n");
		return -1;
	}
	...
	if(vpt[PGNUM(addr)] & (PTE_W | PTE_COW))
	{
		...
		if(r<0)
		{
			cprintf("lib/fork.c: %s: sys_page_map failed 1!\n", __FUNCTION__);
			return -1;
		}
		...
		if(r<0)
		{	
			cprintf("lib/fork.c %s: sys_page_map failed 2!\n", __FUNCTION__);
			return -1;
		}
	}
	else
	{
		...
		if(r<0)
		{
			cprintf("lib/fork.c: %s: sys_page_map failed 3!\n", __FUNCTION__);
			return -1;
		}
	}
...
fork(void)
{
	...
	if(i<0)
	{
		panic("set pgfault_upcall error %e\n", i);
	}

	i = sys_env_set_status(childid, ENV_RUNNABLE);
	if(i<0)
	{
		panic("set child status to runnable error %e\n", i);
	}
